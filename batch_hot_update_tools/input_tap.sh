...
if [ "0" -ne "$?" ]; then
   echo "IP:$IP down." 
   echo "IP:$IP down" >> ssh_down.txt
   exit 2;
fi

(ssh $HOSTCHECK $IP "input tap 764 530; sleep 5; input tap 738 621; sleep 60; input tap 764 530; sleep 5;input tap 738 621" > /dev/null 2>/dev/null)&
