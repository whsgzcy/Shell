...
APK_ANDROID_PATH=/data/lxc_volumns/system/bin
...
if [ "0" -ne "$?" ]; then
   echo "IP:$IP down." >> upload_down.out 
   exit 2;
fi
...
#ssh $HOSTCHECK $IP "echo $ULIMIT  >> /data/lxc_volumns/system/bin/install-recovery.sh" > /dev/null 2>/dev/null

ssh $HOSTCHECK $IP 'sed -i "15aulimit -Sn 4096"  /data/lxc_volumns/system/bin/install-recovery.sh' > /dev/null 2>/dev/null

#ssh $HOSTCHECK $IP "rm -rf /data/lxc_volumns/system/bin/install-recovery.sh" > /dev/null 2>/dev/null
...
#if [ "0" -ne "$?" ]; then
#    echo "IP:$IP scp failed." >> updatefile.out
#    exit 2;
#fi

#ssh $HOSTCHECK $IP "chown root:root /data/lxc_volumns/system/bin/install-recovery.sh ; chmod 750 /data/lxc_volumns/system/bin/install-recovery.sh" > /dev/null 2>/dev/null
