...
if [ "0" -ne "$?" ]; then
   echo "IP:$IP down." 
   echo "IP:$IP down" >> $(date "+%Y-%m-%d")-down_ulimit.txt
   exit 2;
fi

ssh $HOSTCHECK $IP "source /data/migu_etc/ulimit.sh"

#sleep 1

#ssh $HOSTCHECK $IP "echo $IP; ulimit -a | grep nofiles" >>  $(date "+%Y-%m-%d")-ulimit.txt 
