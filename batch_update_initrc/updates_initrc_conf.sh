...
ssh $HOSTCHECK $IP 'cd /data/lxc_volumns; umount /data/lxc_volumns/root; busybox mount -t ext4 -o loop root_container.img root; sed -i "415asetrlimit 7 4096 4096"  /data/lxc_volumns/root/init.rc' > /dev/null 2>/dev/null
